#!/usr/bin/env python3
"""
Nano Banana 2 API 工具（完全按照官方文档）
https://kie.ai/nano-banana-2

用法:
  python3 nano_banana_2_official.py --prompt "your prompt" --wait
  python3 nano_banana_2_official.py --prompt "your prompt" --image_input url1 url2 --wait
"""

import sys
import json
import os
import argparse
import time
import urllib.request

# API 配置
API_BASE = "https://api.kie.ai/api/v1"
API_KEYS = [
    "ea124e82c156043a3c6df388436e70a2",
    "2e30ef813eb98a563cd01b7596f3235d",
    "997cbe0fdb6f9afb14b15c5174e37d9a"
]
API_KEY = API_KEYS[0]  # 默认使用第一个

def create_task(
    prompt: str,
    image_input: list = None,
    aspect_ratio: str = "auto",
    google_search: bool = False,
    resolution: str = "1K",
    output_format: str = "jpg",
    callback_url: str = None
) -> dict:
    """创建 Nano Banana 2 任务"""
    url = f"{API_BASE}/jobs/createTask"
    headers = {
        'Authorization': f'Bearer {API_KEY}',
        'Content-Type': 'application/json'
    }
    
    data = {
        "model": "nano-banana-2",
        "input": {
            "prompt": prompt,
            "aspect_ratio": aspect_ratio,
            "google_search": google_search,
            "resolution": resolution,
            "output_format": output_format
        }
    }
    
    # 添加 image_input（如果提供）
    if image_input:
        data["input"]["image_input"] = image_input
    
    # 添加 callback_url（如果提供）
    if callback_url:
        data["callBackUrl"] = callback_url
    
    req = urllib.request.Request(
        url,
        data=json.dumps(data).encode('utf-8'),
        headers=headers,
        method='POST'
    )
    
    try:
        with urllib.request.urlopen(req, timeout=30) as response:
            result = json.loads(response.read().decode('utf-8'))
            return result
    except urllib.error.HTTPError as e:
        error_body = e.read().decode('utf-8')
        return {"error": f"HTTP {e.code}", "detail": error_body}
    except Exception as e:
        return {"error": str(e)}

def query_task(task_id: str) -> dict:
    """查询任务状态"""
    url = f"{API_BASE}/jobs/recordInfo?taskId={task_id}"
    headers = {'Authorization': f'Bearer {API_KEY}'}
    
    req = urllib.request.Request(url, headers=headers)
    
    try:
        with urllib.request.urlopen(req, timeout=15) as response:
            result = json.loads(response.read().decode('utf-8'))
            return result
    except urllib.error.HTTPError as e:
        error_body = e.read().decode('utf-8')
        return {"error": f"HTTP {e.code}", "detail": error_body}
    except Exception as e:
        return {"error": str(e)}

def wait_for_completion(task_id: str, max_wait: int = 300, interval: int = 12) -> dict:
    """等待任务完成"""
    start_time = time.time()
    
    while time.time() - start_time < max_wait:
        result = query_task(task_id)
        
        if "error" in result:
            return result
        
        state = result.get("data", {}).get("state")
        
        if state == "success":
            return result
        elif state == "fail":
            return {"error": "Task failed", "detail": result}
        
        print(f"⏳ 状态: {state}")
        time.sleep(interval)
    
    return {"error": "Timeout", "detail": f"Task not completed after {max_wait}s"}

def save_result(result: dict, output_file: str):
    """保存结果到 JSON"""
    with open(output_file, 'w') as f:
        json.dump(result, f, indent=2, ensure_ascii=False)
    print(f"💾 结果已保存: {output_file}")

def main():
    parser = argparse.ArgumentParser(description="Nano Banana 2 API 工具（官方版本）")
    parser.add_argument("--prompt", help="生成提示词（最多20000字符）")
    parser.add_argument("--image_input", nargs="+", help="参考图片 URL（最多14张，每张最大30MB）")
    parser.add_argument("--aspect_ratio", default="auto", 
                       choices=["1:1", "1:4", "1:8", "2:3", "3:2", "3:4", "4:1", "4:3", "4:5", "5:4", "8:1", "9:16", "16:9", "21:9", "auto"],
                       help="宽高比")
    parser.add_argument("--google_search", action="store_true", help="启用 Google 搜索")
    parser.add_argument("--resolution", default="1K", choices=["1K", "2K", "4K"], help="分辨率")
    parser.add_argument("--output_format", default="jpg", choices=["jpg", "png"], help="输出格式")
    parser.add_argument("--callback_url", help="回调 URL")
    parser.add_argument("--output", default="nano_banana_2_result.json", help="结果保存路径")
    parser.add_argument("--wait", action="store_true", help="等待任务完成")
    parser.add_argument("--query", help="查询指定任务ID的状态")
    
    args = parser.parse_args()
    
    # 查询模式
    if args.query:
        print(f"🔍 查询任务: {args.query}")
        result = query_task(args.query)
        
        if "error" in result:
            print(f"❌ 查询失败: {result['error']}", file=sys.stderr)
            sys.exit(1)
        
        state = result.get("data", {}).get("state")
        print(f"📊 状态: {state}")
        
        if state == "success":
            result_json = json.loads(result["data"]["resultJson"])
            print(f"✅ 结果 URL:")
            for url in result_json.get("resultUrls", []):
                print(f"  - {url}")
        
        print(json.dumps(result, indent=2, ensure_ascii=False))
        sys.exit(0)
    
    # 创建任务
    print("🎨 创建 Nano Banana 2 任务...")
    result = create_task(
        prompt=args.prompt,
        image_input=args.image_input,
        aspect_ratio=args.aspect_ratio,
        google_search=args.google_search,
        resolution=args.resolution,
        output_format=args.output_format,
        callback_url=args.callback_url
    )
    
    if "error" in result:
        print(f"❌ 创建失败: {result['error']}", file=sys.stderr)
        if "detail" in result:
            print(f"详情: {result['detail']}", file=sys.stderr)
        sys.exit(1)
    
    task_id = result.get("data", {}).get("taskId")
    print(f"✅ 任务已创建: {task_id}")
    
    # 等待完成
    if args.wait:
        print("\n⏳ 等待生成完成...")
        result = wait_for_completion(task_id)
        
        if "error" in result:
            print(f"❌ 生成失败: {result['error']}", file=sys.stderr)
            sys.exit(1)
        
        result_json = json.loads(result["data"]["resultJson"])
        result_urls = result_json.get("resultUrls", [])
        
        print(f"\n✅ 生成完成！")
        print(f"📸 结果 URL:")
        for url in result_urls:
            print(f"  - {url}")
        
        # 保存结果
        save_result(result, args.output)
    else:
        print(f"\n💡 使用以下命令查询任务状态:")
        print(f"  python3 nano_banana_2_official.py --query {task_id}")

if __name__ == "__main__":
    main()
